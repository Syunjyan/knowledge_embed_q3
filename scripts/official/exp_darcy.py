...
parser.add_argument("--phys", type=str, default="auto", help="none|pde|match|hard|dyn")
...
if args.phys == "auto":
    args.phys = _PHYS.get(args.group.upper(), "none")
...
def main():
    ...
    print("group", args.group, "phys", args.phys)
    ...
    if eval:
        ...
        with torch.no_grad():
            for x, fx, y in test_loader:
                ...
                if args.phys == "hard":
                    out = apply_dirichlet(out, s)
                rel_err += myloss(out, y).item()
        rel_err /= ntest
        print("rel_err:{}".format(rel_err))
        write_summary(args.summary_json, {"case": "darcy", "group": args.group, "phys": args.phys, "rel_err": rel_err})
        return
    ...
    for ep in range(args.epochs):
        ...
        for x, fx, y in train_loader:
            ...
            pde = u_phys.new_zeros(())
            if args.phys in {"pde", "match", "dyn"}:
                a_phys = x_normalizer.decode(fx)
                if args.phys == "match":
                    pde = darcy_pde_match(u_phys, y, a_phys, dx, s)
                    loss = loss + phys_w * pde
                elif args.phys == "dyn":
                    pde = darcy_pde_residual(u_phys, a_phys, dx, s)
                    loss = loss + dynamic_weight(l2loss, pde, cap=args.unlock) * pde
                elif phys_w > 0:
                    pde = darcy_pde_residual(u_phys, a_phys, dx, s)
                    loss = loss + phys_w * pde
            loss.backward()
        ...
        print(
            "Epoch {} Reg : {:.5f} Train loss : {:.5f} PDE : {:.5f} phys_w : {:.4f}".format(
                ep, reg, train_loss, pde_acc, phys_w
            )
        )
        ...
        rel_err /= ntest
        print("rel_err:{}".format(rel_err))
        history.append({"epoch": ep, "train_loss": train_loss, "reg": reg, "pde": pde_acc, "rel_err": rel_err, "phys_w": phys_w})
        write_summary(
            args.summary_json,
            {
                "case": "darcy",
