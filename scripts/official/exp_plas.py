...
parser.add_argument(
    "--data_path",
    type=str,
    default=str(ROOT / "data" / "official" / "plas_N987_T20.mat"),
)
...
def main():
    ...
    print("group", args.group, "official plasticity has no extra constitutive PDE in this wrapper")
    ...
    if eval:
        ...
        with torch.no_grad():
            for x, tim, fx, yy in test_loader:
                ...
                for t in range(T):
                    ...
                    pred = im.unsqueeze(-1) if t == 0 else torch.cat((pred, im.unsqueeze(-1)), -1)
        ...
        write_summary(args.summary_json, {"case": "plas", "group": args.group, "rel_err": test_l2_full / ntest})
        return
    ...
    for ep in range(args.epochs):
        ...
        print(
            "Epoch {} , train_step_loss:{:.5f} , test_step_loss:{:.5f} , test_full_loss:{:.5f}".format(
                ep,
        ...
        history.append(
            {
                "epoch": ep,
                "train_step": train_l2_step / ntrain / T,
        ...
        write_summary(
            args.summary_json,
            {
                ...
                "test_step": test_l2_step / ntest / T,
                "history": history,
            },
        )
        if ep % 100 == 0:
